...
const CategoryList = () => {
    ...
    return (
        <div className='container mx-auto p-4'>
            <div className='flex items-center gap-2 justify-between overflow-scroll scrollbar-hide'>
                {
                    loading ? (
                        ...
                        categoryProduct.map((product, index) => {
                            return (
                                <Link to={`/product-category/${product?.brandName}`} className='cursor-pointer ' key={product?.brandName + index}>
